...
In incident response it is common to check connections to determine if a user actually connected to a malicious IP/host. Not every organization has access to a network security monitoring stack to get items like Zeek logs. However, one thing that would be far easier to implement or that is already implemented in the organization is sysmon. For those unfamiliar "System Monitor (Sysmon) is a Windows system service and device driver that, once installed on a system, remains resident across system reboots to monitor and log system activity to the Windows event log." ([Sysmon Documentation][sysmon-docs], 2025). 

Event ID 3 will give you many useful event items about the connection that was established with the remote server, but why I think it is a super power to enable this Event is the ability to see the file name that established the connection (full event objects here: [Sysmon Event ID 3][event-id-3]). Triaging is made much more simple, and when you see X false positives a day that time its takes for temporal cross validation of network logs and event logs eats away at you, especially when it is the 89th false positive you checked on today. 

There is some leg work to be done aside from Sysmon installation for these event logs. If you wish to access these events you also have to enable that event in your sysmon configuration file during installation. To see how to install sysmon and access a very good prebuilt config file visit [Swift on Security Sysmon Configuration and Installation][swift-config].
...
![Create a notebopl ]({{ "/assets/images/vc-notebooks.png" | relative_url }})
...
![Threatfox Hit]({{ "/assets/images/threatfox-hit.png" | relative_url }})
